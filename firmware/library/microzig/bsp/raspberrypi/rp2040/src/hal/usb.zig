...
pub const F = struct {
    ...
    pub fn clear_EP2_OUT_buffer() void {
        // Write one to bit 5 to clear EP2_OUT buffer
        const buff_raw = peripherals.USBCTRL_REGS.BUFF_STATUS.raw;
        std.log.info("Buffer value before: {x}", .{buff_raw});
        //peripherals.USBCTRL_REGS.BUFF_STATUS.modify(.{ .EP2_OUT = 1 });
        peripherals.USBCTRL_REGS.BUFF_STATUS.write_raw(buff_raw | 0x30);
        std.log.info("Buffer value: {x}", .{peripherals.USBCTRL_REGS.BUFF_STATUS.raw});
        //modify_endpoint_control(2, .{});
    }
...
pub fn next(self: *usb.EPBIter) ?usb.EPB {
    ...
    // Find the next buffer with data
    ...
    // Re-arm the buffer for the next reception
    F.usb_start_rx(ep_addr, endpoint.max_packet_size);
